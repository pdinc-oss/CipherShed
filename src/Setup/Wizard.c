		...
		case INTRO_PAGE:
			{
				...
				SetWindowTextW (GetDlgItem (GetParent (hwndDlg), IDC_BOX_INFO), L"You must accept these license terms before you can use, extract, or install CipherShed.");
	...
	case WM_COMMAND:
		...
		if (lw == IDC_NEXT)
		{
			if (nCurPageNo == INTRO_PAGE)
			{
				...
				{
					WarningDirect (L"Warning: Please note that this may be the last version of CipherShed that supports Windows 2000. If you want to be able to upgrade to future versions of CipherShed (which is highly recommended), you will need to upgrade to Windows XP or a later version of Windows.\n\nNote: Microsoft stopped issuing security updates for Windows 2000 to the general public on 7/13/2010 (the last non-security update for Windows 2000 was issued to the general public in 2005).");
					...
					{
						ErrorDirect (L"CipherShed requires Update Rollup 1 for Windows 2000 SP4 to be installed.\n\nFor more information, see http://support.microsoft.com/kb/891861");
						AbortProcessSilent ();
					}
